...
import { ensureBookPdfLocal, webViewUriForLocalPdf } from '../../services/localPdfCache';
...
export default function BookPdfViewerScreen() {
  const router = useRouter();
  const { bookId, title } = useLocalSearchParams<{ bookId: string; title?: string }>();
  const bid = Number.parseInt(bookId ?? '', 10);
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (Platform.OS === 'web') {
        ...
        if (!token) {
          throw new Error('Sign in to view this book.');
        }
        const url = `${FULL_API_URL}/books/library/${bid}/file`;
        ...
        const { localFileUri: local, fromOfflineCache } = await ensureBookPdfLocal(bid, () => apiClient.getToken());
  ...
  const onShare = async () => {
    ...
    try {
      ...
      await Sharing.shareAsync(localFileUri, { mimeType: 'application/pdf', dialogTitle: title || 'Textbook' });
    } catch (e) {
  ...
  const headerTitle = title ? String(title) : `Book #${bid}`;
